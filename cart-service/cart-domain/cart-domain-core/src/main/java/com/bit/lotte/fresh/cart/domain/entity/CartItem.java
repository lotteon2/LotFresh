...
public class CartItem extends BaseEntity<CartItemCompKey>{
  ...
  public void removeCartItem(ProductId productId){
      if(!product.getEntityId().equals(productId)) throw new CartDomainException("카트에 존재하지 않는 상품입니다.");
    }
  ...
  {
    if(selectedQuantity <= 0 ) throw new CartDomainException("1개 이상을 추가해주세요.");
  }
